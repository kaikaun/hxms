...
comps = 5         # Maximum number of peaks for Infinite Gaussian Mixture Model
alpha = 100      # Alpha factor for Infinite Gaussian Mixture Model
...
def main():
	...
	gm = sklearn.mixture.DPGMM(n_components=comps, alpha=alpha)
	for file in files:
		pts = []
		high_I = 0
		f = open(file, 'r')
		...
		gm.fit(pt)
		gm_clusters = len(np.unique(gm.predict(pt)))

		print file+' MS %d'%ms_clusters+' DB %d'%db_clusters+' GMM %d'%gm_clusters+' max %.3f'%high_I
	return 0
